...
if is_cvmfs_needed "$glidein_config" ; then
    ...
    if [[ $gwms_cvmfsexec_mode -eq 3 ]]; then
        ...
        # echo "work_dir (inside cvmfs_setup_new.sh) set to $work_dir"
        export GLIDEIN_CVMFS_CONFIG_REPO="$GLIDEIN_CVMFS_CONFIG_REPO"
        export GLIDEIN_CVMFS_REPOS="$GLIDEIN_CVMFS_REPOS"
        # echo "GWMS_STARTUP_SCRIPT set to $GWMS_STARTUP_SCRIPT"
        ...
        echo "Before the mount_point variable assignment... (in mode 2)"
        ...
        if [[ -n "$mount_point" && "$mount_point" != TARGET* ]]; then
            ...
            if [[ -n "$mount_point" && "$mount_point" != /cvmfs ]]; then
                ...
                echo "CVMFS_MOUNT_DIR set to $CVMFS_MOUNT_DIR (in mode 2)"
            fi
        fi
        echo "CVMFS_MOUNT_DIR is $(printenv CVMFS_MOUNT_DIR) in mode 2"

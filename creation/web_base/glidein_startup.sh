...
#   echo "inside glidein_exit() function..."
...
#   echo "at the end of the glidein_exit() function..."
  log_write "glidein_startup.sh" "text" "glidein is about to exit with retcode $1" "info"
  send_logs_to_remote

  glidien_cleanup
  # echo "Before the print_tail() function..."
  print_tail "$1" "${final_result_simple}" "${final_result_long}"
    ...
    if [[ "${ffb_file_type}" = "exec" || "${ffb_file_type}" = "exec:"* ]]; then
        ...
        if [ "${ffb_id}" = "main" ] && [ "${ffb_target_fname}" = "${last_script}" ]; then  # last_script global for simplicity
            ...
            if [[ "${ffb_file_type}" = "exec:s" ]]; then
                ...
                echo "${ffb_outname}" glidein_config "${ffb_id}"
                "${ffb_outname}" glidein_config "${ffb_id}"
            fi
            ret=$?
            END=$(date +%s)
        ...
        echo "%%%%% inside ffb_file_type=wrapper %%%%%" 1>&2
        echo "%%% wrapper_list: $wrapper_list (before) %%%" 1>&2
        echo "%%% ffb_file_type: $ffb_file_type %%%" 1>&2
        echo "%%% ffb_outname: $ffb_outname %%%" 1>&2
        echo "${ffb_outname}" >> "${wrapper_list}"
        echo "%%% wrapper_list: $wrapper_list (after) %%%" 1>&2
...
if [[ -z "$gwms_cvmfs_reexec" ]]; then
    # if GWMS_CVMFS_REEXEC is empty, then this script is being invoked the first time...
    ...
    # echo "export GWMS_STARTUP_SCRIPT=${GWMS_STARTUP_SCRIPT}" >> /tmp/glidein_env.env
    ...
    # echo "second argument, operation_mode set to $operation_mode"
    ...
    # echo "export GLIDEIN_WORK_DIR=${GLIDEIN_WORK_DIR}" >> /tmp/glidein_env.env
    export GLIDEIN_WORK_DIR=${GLIDEIN_WORK_DIR}
    ...
    export GWMS_BIN_DIR=${gwms_bin_dir}
    ...
    # echo "export GWMS_SUBDIR=${GWMS_SUBDIR}" >> /tmp/glidein_env.env
    export GWMS_SUBDIR=${GWMS_SUBDIR}
    # echo "export GWMS_DIR=${GWMS_DIR}" >> /tmp/glidein_env.env
    ...
    # echo "export GWMS_MAIN_DIR=${main_dir}" >> /tmp/glidein_env.env
    ...
    # echo "export GWMS_ENTRY_DIR=${entry_dir}" >> /tmp/glidein_env.env
    export GWMS_ENTRY_DIR=${entry_dir}
    ...
    if [ -n "${client_repository_url}" ]; then
        ...
        # echo "export GWMS_CLIENT_DIR=${client_dir}" >> /tmp/glidein_env.env
        export GWMS_CLIENT_DIR=${client_dir}
        ...
        # echo "export GWMS_CLIENTGROUP_DIR=${client_group_dir}" >> /tmp/glidein_env.env
        export GWMS_CLIENTGROUP_DIR=${client_group_dir}
    fi
    ...
    # echo "export GLIDEIN_CONFIG=${glidein_config}" >> /tmp/glidein_env.env
    ...
    echo "before log_setup, glidein_config set to $glidein_config"
    ...
    # echo "reached here...description file logic"
        gs_id_descript_file="$(get_descript_file ${gs_id})"
    # echo "gs_id_descript_file set to $gs_id_descript_file"
        ...
        # echo "reached here...signature file logic"
    ...
    echo "last_script (line 2073) set to $last_script"
    ...
    do
        ...
        if [ "$gs_file_id" = "main at_file_list" ]; then
            ...
            # echo "%%%%% gs_id_work_dir: $gs_id_work_dir (first for loop)"
            # echo "%%%%% gwms_exec_dir: $gwms_exec_dir (first for loop)"
...
if [[ -n "$gwms_cvmfs_reexec" && "$gwms_cvmfs_reexec" == "yes" ]]; then
    ...
    # printenv CLIENT_REPOSITORY_URL
    # printenv CLIENT_REPOSITORY_GROUP_URL
    ...
    if [[ ${status} -eq 0 ]]; then
        ...
        # echo "..... wrapper_list (inside reinvocation): $wrapper_list"
        gwms_exec_dir=$(printenv GWMS_EXEC_DIR | sed "s/ //g")
        # echo "..... gwms_exec_dir (inside reinvocation): $gwms_exec_dir"
    fi
    ...
    #     loginfo "CVMFS config repository already mounted!"
    #     # continue
    #     :
    # else
    #     # mounting the configuration repo (pre-requisite)
    #     loginfo "Mounting CVMFS config repo now..."
    #     $CVMFSMOUNT "$cvmfs_config_repo"
    # fi
    # # declare -a additional_cvmfs_repos
    # repos=($(echo $cvmfs_add_repos | tr ":" "\n"))
    # # echo "${repos[@]}"
    ...
    # for repo in "${repos[@]}"
    # do
    #     # echo "Repository being mounted: $repo"
    #     $CVMFSMOUNT "$repo"
    # done

    # # see if all the repositories got mounted
    # num_repos_mntd=$(df -h | grep /cvmfs | wc -l)
    # # echo "num_repos_mntd set to $num_repos_mntd"
    # total_num_repos=$(( ${#repos[@]} + 1 ))
    # # echo "total_num_repos set to $total_num_repos"
    ...
    # re-source all the scripts as it'd have been done in the first invocation
    extract_all_data

    # added after checking stderr log (job.7549.0.err) which reported the following:
    # log_write: missing logging configuration in (/var/lib/condor/execute/dir_955432/condor_exec.exe); perhaps forgot to call log_setup before?
    log_setup "${glidein_config}"
fi  

echo "*************** BEFORE THE SECOND FOR LOOP ***************"
# echo "CVMFS_MOUNT_DIR set to $(printenv CVMFS_MOUNT_DIR) [glidein_startup.sh]"
# echo "work_dir (after reading from env) variable: $work_dir"
# echo "gwms_cvmfsexec_mode (after reading from env) set to $gwms_cvmfsexec_mode"
# echo "gwms_cvmfs_reexec (after reading from env) set to $gwms_cvmfs_reexec"
...
do
    ...
    if [ -z "${client_repository_url}" ]; then
        # echo "%%%% client_repository_url: $client_repository_url %%%%" 1>&2
        if [ "${gs_id}" = "client" ]; then
            # echo "%%% gs_id: $gs_id %%%" 1>&2
            # no client file when no client_repository
            continue
        fi
    fi
    ...
    if [ "$gs_file_id" = "main at_file_list" ]; then
        ...
        # TODO: gwms25073 this is a workaround until there is an official designation for setup script fragments
        # echo "%%%%% gs_id_work_dir: $gs_id_work_dir (second for loop)"
        # echo "%%%%% gwms_exec_dir: $gwms_exec_dir (second for loop)"
        [[ -e "${gs_id_work_dir}/setup_prejob.sh" ]] && { cp "${gs_id_work_dir}/setup_prejob.sh" "$gwms_exec_dir"/prejob/ ; chmod a-x "$gwms_exec_dir"/prejob/setup_prejob.sh ; }
    fi
done
...
(( validation_time=last_startup_time-startup_time ))    # keep the let expr as-is until Franco's PR is merged as it would lead to too many conflicts to fix
...
(( last_script_time= last_startup_end_time - last_startup_time ))
...
# if [[ "$gwms_cvmfs_reexec" == "yes" ]]; then
#     # echo "************************* ENDING LOGIC *************************"
#     unset gwms_cvmfs_reexec
#     unset gwms_cvmfsexec_mode
#     echo "-------- VARIABLES ARE UNSET --------":q
# fi

# echo "gwms_cvmfs_reexec set to $gwms_cvmfs_reexec"
# echo "gwms_cvmfsexec_mode set to $gwms_cvmfsexec_mode"
echo ".....almost at the end of glidein_startup script....."
#########################
# clean up after I finish
glidein_exit ${ret}
# else
    # echo "something is wrong"
    # echo "Back in first invocation of glidein_startup"
# fi

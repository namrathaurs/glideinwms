...
perform_system_check() {
    ...
    if [[ -f '/etc/redhat-release' ]]; then
        ...
        GWMS_OS_DISTRO="rhel"
        # TODO: source the release file after determining if the OS is a non-RHEL derivative
...
prepare_for_cvmfs_mount () {
    ...
    if [[ ! -d "$glidein_cvmfsexec_dir" && ! -f "${glidein_cvmfsexec_dir}/${dist_file}" ]]; then
        ...
        logerror "Error occurred during preparing for cvmfs setup: None of the available cvmfsexec distributions is compatible with the worker node specifications."
